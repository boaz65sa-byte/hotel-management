...
## היררכיית הרשאות המלאה

### טבלת הרשאות

| תפקיד | פתיחת קריאה | שיבוץ עובד | סגירה מלאה | Dashboard | ניהול משתמשים | גישה לכל המלונות |
|-------|------------|------------|------------|-----------|--------------|-----------------|
| `super_admin` | ✅ | ✅ | ✅ | ✅ קריאה+כתיבה | ✅ | ✅ |
| `hotel_admin` | ✅ | ✅ | ✅ | ✅ קריאה+כתיבה | ✅ במלון שלו | ❌ |
| מנהל מחלקה | ✅ | ✅ במחלקתו | ✅ | ✅ קריאה בלבד | ❌ | ❌ |
| סגן מנהל | ✅ | ✅ במחלקתו | ❌ | ✅ קריאה בלבד | ❌ | ❌ |
| עובד | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |

### כללים מיוחדים

**super_admin:**
- רואה ויכול לשנות נתונים לכל מלון
- מעניק/שולל הרשאת `hotel_admin`
- גישה דרך Admin Web בלבד (Next.js)

**hotel_admin:**
- ניהול משתמשים: יצירה, עריכה, הפעלה/כיבוי — דרך Web **ו**-Mobile
- יכול לתת כל תפקיד **מלבד** `hotel_admin` ו-`super_admin`
- רואה ויכול לשנות כל נתון במלון שלו

**מנהל מחלקה:**
- סגירה מלאה של קריאה (final approval)
- Dashboard — קריאה בלבד

**סגן מנהל:**
- כל הרשאות מנהל **חוץ** מסגירה מלאה
- יכול לסמן "הושלם" — אבל המנהל חייב לאשר סגירה

**עובד:**
- פותח קריאות לכל מחלקה
- מעדכן קריאות שמשובצות אליו
- אין גישה לDashboard

### זרימת סגירת קריאה
```
עובד → "סיימתי" (+ תמונת אחרי)
  ↓
מנהל מחלקה → רואה "ממתין לאישור"
  ↓
מנהל לוחץ "סגור קריאה" → סגורה ✅ + נשמר בהיסטוריה
```

### Dashboard — מה מוצג

**כרטיסי KPI:**
- סה"כ קריאות פתוחות / סגורות / חירום
- זמן טיפול ממוצע (לפי מחלקה)
- % עמידה ב-SLA

**גרפים:**
- קריאות לפי חודש / יום (line chart)
- קריאות לפי מחלקה (pie/bar chart)
- קריאות שנפתחו vs נסגרו (stacked bar)

**טבלת היסטוריה:**
- כל קריאה: מספר, מחלקה, פותח, מטפל, זמן פתיחה, זמן סגירה, משך טיפול
- ניתן לסינון לפי תאריך / מחלקה / דחיפות
- כפתורי פעולה אקטיביים (לפתוח קריאה, לשנות שיבוץ)

**גישה:**
- `super_admin`: כל המלונות, קריאה+כתיבה
- `hotel_admin`: מלון שלו, קריאה+כתיבה
- מנהל מחלקה + סגן: מלון שלהם, קריאה בלבד
...
CREATE POLICY hotel_admin_managed_by_super
  ...
  USING (
    ...
    OR (
      ...
      AND role NOT IN ('hotel_admin','super_admin')
    )
  );
```
