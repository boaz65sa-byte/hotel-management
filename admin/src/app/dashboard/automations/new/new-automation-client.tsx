...
export default function NewAutomationClient({ lockedHotelId }: { lockedHotelId?: string | null }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    const hid = lockedHotelId ?? form.hotel_id.trim()
    if (!hid) {
      setError('חסר מלון')
      setSaving(false)
      return
    ...
    const res = await fetch('/api/automations', {
      ...
      body: JSON.stringify({ ...form, hotel_id: hid }),
    })
    if (res.ok) {
      router.push(lockedHotelId ? `/dashboard/automations?hotel_id=${lockedHotelId}` : '/dashboard/automations')
    } else {
        ...
        {!lockedHotelId ? (
          <div>
            ...
            <input
              ...
              required
              placeholder="UUID של המלון"
            />
          </div>
        ) : (
          <p className="text-sm text-gray-600">האוטומציה תישמר עבור המלון אליו אתם מחוברים.</p>
        )}
