...
### ✅ amenity_orders UI — נבנה ונבדק ויזואלית (commit 38d3995), ממתין לפריסה ידנית
נוסף טאב "הזמנות" ל-`ReceptionHomeScreen` ול-`ManagerHomeScreen` (`lib/features/amenity_orders/`) — סינון סטטוס (הכל/פתוחות/אושרו/נמסרו), פעולות אישור/מסירה/ביטול, שם וקטגוריה נטענים בנפרד מ-`hotel_amenities` ב-lookup לפי id (realtime `.stream()` לא תומך ב-join). **נבדק בפועל**: התחברתי כ-manager@hotel.com על build מקומי (אחרי תיקון ה-LocalDb try/catch, ראה למטה), נלחץ הטאב, נבדק state ריק ("אין הזמנות") ועם נתוני בדיקה אמיתיים — עובד תקין. `flutter analyze` נקי, `flutter build web --release` הצליח.
**צעד הבא**: `flutter build web --release` (כבר בוצע, ב-`build/web`) → גרירה ל-Netlify Drop לפי `LINKS.md` → זהו, זה כבר נבדק. שים לב: `amenities_ordering` כבוי כרגע לכל המלונות (כולל האמיתי) — אין דחיפות תפעולית. **הכפתור להפעלת המודול כבר קיים** בעמוד עריכת מלון (`hotel-form.tsx`, commit `155b123` — נבנה בסבב קודם, לא על ידי).

### 🔍 maintenance_manager — בטיפול אצל session מקביל (hotel-guest-app-f6)
תוך כדי הבדיקה המקומית מצאתי **תיקון קריטי לא-commit-ed** ב-`lib/main.dart`: `await LocalDb.instance` (SQLite pre-warm) לא היה עטוף ב-try/catch, וכשל שקט שלו (`DatabaseException(getDatabasesPath is null)` — sqflite_common_ffi_web) חסם את **runApp() לגמרי, לכל תפקיד**, לא רק maintenance_manager — זה גרם למסך לבן בכל טעינה ראשונית באתרים ללא ה-worker/headers הנכונים. זה תוקן (uncommitted) על ידי session מקביל שכבר עובד על הבאג הספציפי (401+406 ב-"בקשות"), עם השערה של JWT מיושן. גם מצאתי leads נוסף: `PGRST116` (0 rows על `.single()`) ב-`session_timeout.dart` בזמן session-restore — דיווחתי להם, לא בטוח קשור ישירות. **לא נגעתי בקבצים האלה** כדי לא להתנגש.
